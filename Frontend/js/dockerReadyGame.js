window.addEventListener("DOMContentLoaded", async function () {
    ...
    function enablePatternLineSelection(gameId, selectedTileColor, aantalTegels) {
        ...
        placeOnFloorLineBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert('Fout bij plaatsen van tegel op vloerlijn: ' + errorText);
                    hasSelectedTile = false;
                    placeOnFloorLineBtn.remove();
                    placeOnFloorLineBtn = null;
                }
        ...
        const patternLines = document.querySelectorAll(".pattern-line:not(.floor-line)");
        patternLines.forEach((line) => {
            const newLine = line.cloneNode(true);
            line.parentNode.replaceChild(newLine, line);
        });

        const freshPatternLines = document.querySelectorAll(".pattern-line:not(.floor-line)");
        freshPatternLines.forEach((line) => {
            line.addEventListener("click", async () => {
                ...
                console.log('Color mapping debug:', {
                    ...
                    normalizedColor: normalizedSelectedColor,
                ...
                if (currentPlayer) {
                    ...
                    if (hasTileInWall) {
                        ...
                        errorContainer.style.backgroundColor = '#dc3545';
                        ...
                        errorContainer.innerHTML = `
                            <div style="font-size: 18px; margin-bottom: 15px;">
                                <strong>Ongeldige zet!</strong>
                            </div>
                            <div style="margin-bottom: 15px;">
                                Je kunt geen tegel van deze kleur op deze rij plaatsen omdat er al een tegel van dezelfde kleur op de muur staat.
                            </div>
                            <button style="
                        ...
                        // Add click handler to container to close when clicking outside
                        document.addEventListener('click', function closeError(e) {
                            if (!errorContainer.contains(e.target)) {
                                errorContainer.remove();
                        ...
                        // Do NOT reset selection state since this is an invalid move
                        // Keep the floor line button visible
                        return;
                    }
                }
                ...
                if (existingColor && existingColor !== selectedTileColor.toLowerCase().trim()) {
                    ...
                    if (normalizedExisting !== normalizedSelected && !sameType) {
                        ...
                        alert("Je kunt geen tegels van verschillende kleuren op dezelfde patroonlijn plaatsen.");
                        return;
                    }
                }
                ...
                if (alreadyFilled === maxCapacity) {
                    alert("Deze patroonlijn is al volledig gevuld.");
                    return;
                }

                // Calculate tiles to place and overflow
                const availableSpace = maxCapacity - alreadyFilled;
                const tilesToPlace = Math.min(aantalTegels, availableSpace);
                const overflowCount = aantalTegels - tilesToPlace;

                try {
                    console.log('Pattern line placement attempt:', {
                        gameId,
                        frontendIndex,
                        backendIndex: patternLineIndex,
                        availableSpace,
                        tilesToPlace,
                        overflowCount,
                        selectedTileType
