...
internal class InMemoryTableRepository : ITableRepository
{
    ...
    public IList<ITable> FindTablesWithAvailableSeats(ITablePreferences preferences)
    {
        ...
        List<ITable> tablesWithAvailableSeats = new List<ITable>();
        foreach (var table in _tableDictionary.Values)
        {
            if (table.HasAvailableSeat && table.Preferences.NumberOfPlayers == preferences.NumberOfPlayers && table.Preferences.NumberOfArtificialPlayers == preferences.NumberOfArtificialPlayers)
            {
